---
title: "Analysing Learner Speech and Resulting Applications for EFL Classrooms - A corpus-based contrastive analysis of vowel production of L1 English speakers and L1 Japanese learners of English (Part 2)"
...
This R Notebook shows the analysis of high front and back vowels produced by Japanese learners and L1 speakers of English. This notebook contains the second part of the analysis which focuses on data processing, visualization, and analysis.
...
fdat <- fdat %>%
  ...
  dplyr::group_by(type, tvariety, gender) %>%
...
generate overview (data before processing)

```{r tb0, message=F, warning=F}
tb0 <- fdat %>%
  dplyr::ungroup() %>%
  dplyr::filter(tformants == "F5",
                label == "I" | label == "i:" | label == "U" | label == "u:" ) %>%
  dplyr::group_by(type, label) %>%
  dplyr::summarise(speakers = max(length(table(speaker))), 
                   obs = n()) %>%
  dplyr::mutate(speakers = max(speakers)) %>%
  tidyr::spread(label, obs) %>%
  dplyr::ungroup() %>%
  dplyr::add_row(type = "Total",
...
```{r checkdur, message=F, warning=F}
fdat <- fdat %>%
  # duration
...
ggsave(here::here("images", "dur_IiUu.png"), height = 5,  width = 5, dpi = 320)
dur_IiUu
```


```{r plotdur2, message=F, warning=F}
fdat %>%
  ggplot(aes(x = vowel, y = duration, fill = type)) +
  geom_boxplot()
```
...
```{r plotdur3, message=F, warning=F}
fdat %>%
  dplyr::group_by(type, tvariety, vowel) %>%
...
fdat <- fdat %>% 
  ...
  dplyr::mutate(densF1 = sm.density(barkF1,eval.points=barkF1)$estimate,
                ...
                qnt1F1 = quantile(densF1, .25),
                qnt1F2 = quantile(densF2, .25))
# inspect
nrow(fdat); head(fdat); summary(fdat$densF1)
```

```{r remout, message=F, warning=F}
fdat <- fdat %>%
...
```{r checkden, message=F, warning=F}
wordplot3 <- function(fdat, vwl1, vwl2){
  ...
  scale_fill_manual(values = c("orange", "gray")) +
    labs(x = "Formant 2 (z-transformed)", y = "Formant 1 (z-transformed)")
  return(plt)
  }
```


```{r pIi, message=F, warning=F}
pIi <- wordplot3(fdat = fdat, vwl1 = "I", vwl2 = "i:")
ggsave(here::here("images", "pIi.png"), height = 3,  width = 5, dpi = 320)
pIi
```

```{r pUu, message=F, warning=F}
...
# save
base::saveRDS(fdat, file = here::here("data", "fdat_kiel.rda"))
# inspect
nrow(fdat); head(fdat)
```
...
## Mixed-Effects Model (DV:Duration)

check data

```{r durtb4, message=F, warning=F}
durtb <- fdat %>%
  dplyr::group_by(type, tvariety, vowel) %>%
...
```{r durplot5, message=F, warning=F}
fdat %>%
  dplyr::group_by(type, tvariety, vowel) %>%
...
```{r diffdur2, message=F, warning=F}
dplot <- diff_dat %>%
...
```{r m6, message=F, warning=F}
# add type x vowel
m6 <- update(m2, . ~ . + type * vowel)
# checks
...
```{r pmf1, message=F, warning=F}
sjPlot::tab_model(m0, m10)
```

```{r pmf2, message=F, warning=F}
sjPlot::tab_model(m10)
```

```{r mef, message=F, warning=F}
sjPlot::plot_model(m10) +
  coord_flip(ylim = c(-.1, .1)) 
```


```{r mef2, message=F, warning=F}
...
plot_model(m10, type = "pred", terms = c("type", "vowel"), axis.lim = c(0, .2)) +
  theme_bw() +
  labs(x = "Speaker type", y = "Predicted Duration (ms)", title = "") +
  ...
  theme(legend.position = "top")
# save
ggsave(here::here("images", "ef.png"), height = 3,  width = 4, dpi = 320)
```
...
```{r rep, eval = F, message=F, warning=F}
report::report(m10)
```
...
Schweinberger, Martin. `r format(Sys.time(), '%Y')`. Analysing Learner Speech and Resulting Applications for EFL Classrooms - A corpus-based contrastive analysis of vowel production of L1 English speakers and L1 Japanese learners of English (Part 2). Brisbane: The University of Queensland, School of Languages and Cultures. url: https://slcladal.github.io/praatrf.html (Version `r format(Sys.time(), '%Y.%m.%d')`).

@manual{schweinberger`r format(Sys.time(), '%Y')`japevk2,
